...
(factory => {
  ...
  const VOWEL = /[aeiouáéíóúüÜ]/i; // Vowel
  ...
  return (word = '') => {
    ...
    for (let letter = 0, jump = 0, len = word.length; letter < len; letter += jump, jump = 0) {
      ...
      // Onset
      if (CONSONANT_GROUPS.test(word.substr(letter, 2))) {
        jump += 2;
      } else if (CONSONANT.test(word[letter])) {
        jump += 1;
      }

      // Nucleus
      if (TRIPHTHONG.test(word.substr(letter + jump, 3))) {
        jump += 3;
      } else if (DIPHTHONG.test(word.substr(letter + jump, 2))) {
        jump += 2;
      } else if (VOWEL.test(word[letter + jump]) || /y/.test(word[letter + jump])) {
        jump += 1;
      } else {
      ...
      // Coda
      if (len - (letter + jump) < 2 && CONSONANT.test(word[letter + jump])) {
        jump += 1;
